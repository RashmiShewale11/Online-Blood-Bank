...
?>
	 ...
	 $sql='SELECT * FROM donation where donor_id="'.$_GET['id'].'" ORDER by id DESC LIMIT 1;';
	//$sql='SELECT TOP 1 (payroll_date1) FROM payroll where prisoner_id="'.$_SESSION['prisoner_id'].'"ORDER BY payroll_date1 DESC';
     // $sql='select * from payroll where prisoner_id="'.$_SESSION['prisoner_id'].'"';
//echo $sql;
...
if($days_between <90)
{
	...
	echo 'window.location.href="blooddonate.php";';
	echo '</script>';
}
else

    {
		$to = $_GET['email']; 
		echo $to;
		$message="You are Eligible to Donote blood.Please Donate Blood And Save Life";
		

		$subject = 'Blood bank';
		$headers = 'From: <Blood bank>';
